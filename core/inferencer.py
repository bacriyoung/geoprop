...
# NOTE: No GBlobs import needed for V3.0
...
def process_room_full_pipeline(cfg, model, data, return_all=False):
    ...
    for t in range(rounds):
        ...
        for x in np.arange(min_c[0], max_c[0], 1.5):
            for y in np.arange(min_c[1], max_c[1], 1.5):
                mask_indices = np.where((aug_xyz[:,0]>=x) & (aug_xyz[:,0]<x+2.0) & (aug_xyz[:,1]>=y) & (aug_xyz[:,1]<y+2.0))[0]
                if len(mask_indices) > 50:
                    ...
                    with torch.no_grad():
                        ...
                        feat = torch.cat([b_xyz, b_rgb], dim=1) # [1, 6, N]
                        ...
                        # Forward (using absolute features 'feat')
    ...
    # ... (Post-processing same as V31) ...
    # Simplified copy of post-proc logic
...
def validate_full_scene_logic(model, cfg, all_files):
    ...
    for f in all_files:
        ...
        pred = list(res_dict.values())[-2] # Get last prediction before GT
        evaluator.update(pred, res_dict['GT'])
    _, miou, _ = evaluator.compute()
    return miou

def run_inference(cfg, model):
    # Same as V31 structure
    logger = logging.getLogger("geoprop")
    logger.info(">>> [Phase 2] Final Inference (V3.0 Overfit)...")
    model.eval()
