...
class Embedding(PointModule):
    ...
    def forward(self, point: Point):
        ...
        # Efficiently calculate N-to-M mapping (Inverse Indices) using torch.unique.
        # This has minimal overhead and is mathematically equivalent to SpConv's internal logic.
        # ==================================================================================================
        if "inverse_indices" not in point.keys():
            # 1. Construct keys: (Batch, Z, Y, X)
            # We use int32 to save memory, assuming coordinates fit in int32 (usually true).
            coords_with_batch = torch.cat([point.batch.unsqueeze(-1).int(), point.grid_coord.int()], dim=1)
            
            # 2. Get inverse mapping directly
            # sorted=True ensures the order aligns with SpConv's internal sorted indices.
